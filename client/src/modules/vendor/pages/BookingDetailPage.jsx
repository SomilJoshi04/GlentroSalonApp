...
const BookingDetailPage = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      <button onClick={() => navigate(-1)} className="text-sm text-slate-500 hover:text-primary-600">← Back</button>
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-100">
        ...
        <div className="flex justify-between font-bold mt-3 pt-3 border-t border-slate-200"><span>Total</span><span className="text-primary-600">₹{booking.finalAmount}</span></div>
      </div>
      ...
      {booking.status === 'CONFIRMED' && <button onClick={() => handleAction('complete')} className="w-full py-3 bg-primary-600 text-white rounded-xl font-medium hover:bg-primary-700">Mark Complete</button>}
    </div>
  );
};
