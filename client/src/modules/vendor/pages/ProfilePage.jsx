  ...
  return (
    <div className="max-w-lg mx-auto space-y-6 animate-fade-in">
      ...
      <div className="bg-white rounded-2xl p-8 border border-slate-100 text-center">
        <div className="w-20 h-20 mx-auto bg-gradient-to-br from-primary-400 to-primary-600 rounded-full flex items-center justify-center text-white text-3xl font-bold">{user?.name?.charAt(0)}</div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100 space-y-4">
        <div className="flex justify-between"><h3 className="font-semibold">Details</h3><button onClick={() => setEditing(!editing)} className="text-sm text-primary-600 font-medium">{editing ? 'Cancel' : 'Edit'}</button></div>
        {editing ? (
          <div className="space-y-3">
            ...
            <button onClick={handleSave} disabled={saving} className="w-full py-2.5 bg-primary-600 text-white rounded-xl text-sm font-medium disabled:opacity-50">{saving ? 'Saving...' : 'Save'}</button>
          </div>
        ) : (
