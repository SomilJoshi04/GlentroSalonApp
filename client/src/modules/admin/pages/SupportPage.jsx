    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {chats.length > 0 && (
        <div className="relative mb-2 max-w-xl">
          ...
          <input 
            type="text" 
            placeholder="Search tickets by user name, ID or message..." 
            value={searchQuery}
            ...
            {filteredChats.map(chat => {
              ...
              return (
                <div key={chat._id} onClick={() => navigate(`/admin/support/${chat._id}`)}
                  ...
                  <div className="w-12 h-12 bg-surface-variant rounded-full flex items-center justify-center text-xl text-muted-text flex-shrink-0 group-hover:bg-soft-primary group-hover:text-primary transition-colors font-bold border border-border group-hover:border-primary/20">
                    {participantName.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-on-surface truncate">{participantName}</h4>
                    <p className="text-xs text-primary font-medium mt-0.5">{displayId}</p>
                    <p className="text-sm text-muted-text truncate mt-1">{chat.lastMessage?.content || 'No messages yet'}</p>
                  </div>
                  {chat.lastMessage?.timestamp && (
                    <span className="text-[11px] font-medium text-muted-text flex-shrink-0">
